...
def render_table(headers: list[str], rows: list[list[str]], aligns: list[str] | None = None) -> list[str]:
    """A GitHub table with aligned pipes, which is what awesome-lint requires.

    aligns holds 'left' or 'right' per column; the delimiter row pads with dashes so
    every pipe in the block sits in the same column.
    """
    ...
    def line(cells: list[str]) -> str:
        ...
        for index, cell in enumerate(cells):
            out.append(cell.rjust(widths[index]) if aligns[index] == 'right' else cell.ljust(widths[index]))
        return '| ' + ' | '.join(out) + ' |'

    delimiter = []
    for index, width in enumerate(widths):
        delimiter.append('-' * (width - 1) + ':' if aligns[index] == 'right' else '-' * width)
    return [line(headers), '| ' + ' | '.join(delimiter) + ' |', *(line(row) for row in rows)]


def align_tables(text: str) -> str:
    """Re-pad every Markdown table in `text` so the pipes line up.

    awesome-lint (remark-lint:table-pipe-alignment) requires it, and a contributor
    editing the README by hand should not have to count spaces. Fenced code is left
    alone, and escaped pipes inside a cell are not treated as separators.
    """
    ...
    lines, output, fenced, index = text.split('\n'), [], False, 0
    while index < len(lines):
        ...
        while index < len(lines) and lines[index].lstrip().startswith('|'):
            ...
            index += 1
        if len(block) < 2 or set(block[1].replace('|', '').replace(' ', '')) - set('-:'):
            output.extend(block)
            continue
        header, delimiter, body = cells(block[0]), cells(block[1]), [cells(row) for row in block[2:]]
        aligns = ['right' if cell.endswith(':') else 'left' for cell in delimiter]
        output.extend(render_table(header, body, aligns))
    return '\n'.join(output)
...
def render_catalogue(papers: list[dict], topics: list[dict]) -> str:
    ...
    for topic in topics:
        ...
        body.append([markdown_link(topic['label'], 'tags/' + topic['id'] + '.md'),
                     markdown(topic['description']), str(count)])
    rows = ['## Browse by topic', '',
            f'**{len(papers)} papers · {len(topics)} topics.** A paper can appear in several topics.', '']
    rows.extend(render_table(['Topic', 'Focus', 'Papers'], body, ['left', 'left', 'right']))
    return '\n'.join(rows)


def render_tag(topic: dict, papers: list[dict]) -> str:
    selected = [paper for paper in papers if topic['id'] in paper['tags']]
    lines = [f'# {topic["label"]}', '', topic['description'], '', '[← All topics](../README.md#browse-by-topic)', '', '<!-- Generated by scripts/build.py from data/papers/. Do not edit directly. -->', '', f'{len(selected)} {"paper" if len(selected) == 1 else "papers"}, newest first.', '']
    body = []
    for paper in selected:
        ...
        body.append([identity, markdown(paper['venue']), markdown(paper['notes'][topic['id']])])
    lines.extend(render_table(['Paper', 'Venue', 'Why it is included'], body))
...
def render_page(papers: list[dict], topics: list[dict], collection: dict, root: Path = ROOT) -> str:
    ...
    replacements = {
        ...
        '__FIELD_MAP_MOBILE__': (root / 'site/static/assets/field-map-mobile.svg').read_text(encoding='utf-8'),
...
def build(root: Path = ROOT, check: bool = False) -> None:
    ...
    documents = {readme: align_tables(replace_catalogue(readme.read_text(encoding='utf-8'), render_catalogue(papers, topics)))}
    for topic in topics:
